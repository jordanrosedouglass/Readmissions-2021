...
gt(COPDlength) %>% 
  tab_header(
    title = md("Length of Stay (Days) - COPD - 2018")
  )
```
...
gt(COPDSource) %>% 
  tab_header(
    title = md("Admission Source - COPD - 2018")
  )
```
...
gt(COPDType) %>% 
  tab_header(
    title = md("Admission Type - COPD - 2018")
    )
```
...
ggplot(COPD2, aes(x = Admit.Type, y = perc, fill = Admit.Type)) + 
  ...
  labs(x = "Admission Type",y = "Percent", title = "Admission Type - Visits - COPD - 2018") +
...
ggplot(COPD2, aes(x = Admit.Type, y = perc, fill = Admit.Type)) + 
  geom_bar(stat = "identity", show.legend = FALSE, colour = "Black") + 
  labs(x = "Admission Type",y = "Percent", title = "Admission Type - Readmitted - COPD - 2018") +
  geom_text(aes(label = percent(perc)), position = position_stack(vjust=0.5)) +
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_fill_brewer(palette = "Pastel1")
