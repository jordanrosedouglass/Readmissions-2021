...
gt(COPDlength) %>% 
  tab_header(
    title = md("Length of Stay (Days) - COPD - 2020")
  )
```
...
COPDRace <- data.frame(
  ...
  Black = percent(as.numeric(c(
    ...
    Asian = percent(as.numeric(c(
    COPD_vis %>% filter(Race == "ASIAN") %>% count() / vtotal,
    COPD_read %>% filter(Race == "ASIAN") %>% count() / rtotal
  ))),
    Unknown = percent(as.numeric(c(
    COPD_vis %>% filter(Race == "UNKNOWN") %>% count() / vtotal,
    COPD_read %>% filter(Race == "UNKNOWN") %>% count() / rtotal
  ))),
    Indian = percent(as.numeric(c(
    COPD_vis %>% filter(Race == "AM INDIAN/AK NATIVE") %>% count() / vtotal,
    COPD_read %>% filter(Race == "AM INDIAN/AK NATIVE") %>% count() / rtotal
  ))),
  Ref = percent(as.numeric(c(
    COPD_vis %>% filter(Race == "REFUSES TO ANSWER") %>% count() / vtotal,
    COPD_read %>% filter(Race == "REFUSES TO ANSWER") %>% count() / rtotal
  )))
)

gt(COPDRace) %>% 
  tab_header(
    title = md("Race - COPD - 2020")
  )
```
...
gt(COPDSource) %>% 
  tab_header(
    title = md("Admission Source - COPD - 2020")
  )
```
...
COPDType <- data.frame(
  ...
  Trauma = percent(as.numeric(c(
    COPD_vis %>% filter(Admit.Type == "TRAUMA") %>% count() / vtotal,
    COPD_read %>% filter(Admit.Type == "TRAUMA") %>% count() / rtotal
  )))
)

gt(COPDType) %>% 
  tab_header(
    title = md("Admission Type - COPD - 2020")
    )
```
...
ggplot(COPD2, aes(x = Admit.Type, y = perc, fill = Admit.Type)) + 
  ...
  labs(x = "Admission Type",y = "Percent", title = "Admission Type - Visits - COPD - 2020") +
...
ggplot(COPD2, aes(x = Admit.Type, y = perc, fill = Admit.Type)) + 
  geom_bar(stat = "identity", show.legend = FALSE, colour = "Black") + 
  labs(x = "Admission Type",y = "Percent", title = "Admission Type - Readmitted - COPD - 2020") +
  geom_text(aes(label = percent(perc)), position = position_stack(vjust=0.5)) +
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_fill_brewer(palette = "Pastel1")
