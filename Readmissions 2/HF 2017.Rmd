...
gt(HFlength) %>% 
  tab_header(
    title = md("Length of Stay (Days) - HF - 2017")
  )
```
...
gt(HFSource) %>% 
  tab_header(
    title = md("Admission Source - HF - 2017")
  )
```
...
gt(HFType) %>% 
  tab_header(
    title = md("Admission Type - HF - 2017")
    )
```
...
ggplot(HF2, aes(x = Admit.Type, y = perc, fill = Admit.Type)) + 
  ...
  labs(x = "Admission Type",y = "Percent", title = "Admission Type - Visits - HF - 2017") +
...
ggplot(HF2, aes(x = Admit.Type, y = perc, fill = Admit.Type)) + 
  geom_bar(stat = "identity", show.legend = FALSE, colour = "Black") + 
  labs(x = "Admission Type",y = "Percent", title = "Admission Type - Readmitted - HF - 2017") +
  geom_text(aes(label = percent(perc)), position = position_stack(vjust=0.5)) +
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_fill_brewer(palette = "Pastel1")
